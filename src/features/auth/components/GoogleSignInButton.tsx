...
type GoogleSignInButtonProps = {
  ...
  /** Force-disable OAuth even when Supabase env is set */
  authNotConfigured?: boolean;
};

export function GoogleSignInButton({
  redirectAfterLogin = AUTH_DEFAULT_REDIRECT,
  authNotConfigured: authNotConfiguredProp,
}: GoogleSignInButtonProps) {
  const authNotConfigured = authNotConfiguredProp ?? !isSupabaseAuthConfigured();
    ...
    <div className="space-y-3">
      <div className="relative">
        <div className="absolute inset-0 flex items-center">
          <div className="border-surface-border w-full border-t"></div>
        </div>
        <div className="relative flex justify-center text-sm">
          <span className="text-on-surface-muted bg-black px-2">O continúa con</span>
        </div>
      </div>
