...
export function LeaguePinnedUserRow({
  ...
  return (
    <div className="pointer-events-none sticky bottom-0 z-20 -mx-1 px-1 pt-3 pb-1">
      <div className="h-6 bg-linear-to-t from-surface-via via-surface-via/80 to-transparent" />
      <button
        type="button"
        onClick={onClick}
        className={cn(
      ...
      >
        <div className="w-6 shrink-0 text-center text-sm font-bold text-on-surface-muted">
          ...
          {statusReaction ? (
            ...
            <span
              className={cn(
                ...
                'border-2 border-on-surface-muted bg-emerald-500'
              )}
            />
          )}
        </div>
        ...
        <div className="shrink-0 text-right text-sm font-bold text-on-surface-muted">{weeklyXp} XP</div>
      </button>
    </div>
  );
}
