...
export function FullExamHeader({
  ...
  const exitButtonClass = cn(
    'relative z-[60] shrink-0 rounded-lg bg-white/10 px-4 py-2 text-sm text-white transition-all duration-300',
  ...
  return (
    <div
      className={cn(
        'border-surface-border from-surface-elevated via-surface-elevated sticky top-0 z-50 border-b bg-linear-to-b',
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center gap-4 px-6">
        ...
        <div className="relative z-[60] flex shrink-0 items-center gap-4">
          {showTimer && timeRemaining != null && (
            <div
              aria-live="polite"
              aria-atomic="true"
              aria-label="Tiempo restante del examen"
              className={cn('shrink-0 font-mono text-2xl font-bold tabular-nums', timeColor)}
            >
