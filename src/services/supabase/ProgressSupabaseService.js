...
const ProgressSupabaseService = {
  ...
  async upsert(userId, progressData) {
    const payload = {
      user_id: userId,
      total_attempts:
        progressData.totalAttempts ?? progressData.total_attempts ?? 0,
      total_correct:
        progressData.totalCorrect ?? progressData.total_correct ?? 0,
      percentage: progressData.percentage ?? 0,
      streak_days: progressData.streakDays ?? progressData.streak_days ?? 0,
      last_activity_date:
        progressData.lastActivityDate ?? progressData.last_activity_date,
