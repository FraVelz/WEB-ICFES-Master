...
const UserSupabaseService = {
  ...
  async createUser(userId, userData) {
    const payload = {
      ...
      display_name: userData.displayName || userData.display_name || 'Usuario ICFES',
  ...
  async updateProfile(userId, profileData) {
    ...
    const { data, error } = await supabase.from(TABLE).update(payload).eq('id', userId).select().single();
