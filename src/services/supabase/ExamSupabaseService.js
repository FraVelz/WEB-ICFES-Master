...
const ExamSupabaseService = {
  ...
  async getById(examId) {
    const { data, error } = await supabase.from(TABLE).select('*').eq('id', examId).maybeSingle();
  ...
  async create(userId, examData) {
    ...
    const payload = {
      ...
      score: null,
      total_questions: examData.questions?.length || examData.totalQuestions || 0,
      correct_answers: 0,
      time_spent: 0,
      completed_at: null,
  ...
  async complete(examId, score, correctAnswers, totalQuestions, timeSpent, answers = []) {
    const payload = {
      score,
    ...
    const { data, error } = await supabase.from(TABLE).update(payload).eq('id', examId).select().single();
