...
1. **¿Solo una ruta de App Router?** → `src/app/.../_components/`.
2. **¿Solo una feature** (`auth`, `exam`, `learning`, `logros`, `store`, `user`, `home`)? →
   `src/features/<feature>/components/` (o `hooks/`, `utils/`, `data/` dentro de esa feature).
3. **¿Shell de toda la app** (providers, guards, header del dashboard)? → `src/components/`.
4. **¿UI reutilizada en 2+ features** (Icon, Footer, alertas)? → `src/shared/components/`.
5. **¿Persistencia o integraciones backend?** → `src/services/` (`@/services/persistence`, etc.).
6. **¿Duda?** → Empezar en el scope más pequeño. Subir a `shared/` solo cuando haya **segundo consumidor** fuera
   de esa feature.
