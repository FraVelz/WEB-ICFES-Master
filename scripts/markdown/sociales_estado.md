---
id: sociales_estado
title: Estado y organización política
area: sociales
order: 1
type: lesson
published: true
description: "Comprende las tres ramas del poder público en Colombia: Legislativa (hace leyes), Ejecutiva (administra) y Judicial (juzga). Conoce los organismos de control y sus funciones."
xp: 500
coins: 250
questions:
  - id: sociales_estado_1
    type: opcion_multiple
    question: "Un ciudadano cree que una ley viola sus derechos fundamentales. ¿A qué rama debe acudir?"
    options:
      - "Ejecutiva"
      - "Legislativa"
      - "Judicial"
      - "Electoral"
    correct_answer: 2
    explanation: "Los jueces (Corte Constitucional) deciden si una ley es válida o viola derechos. Se puede usar la tutela para proteger derechos fundamentales."
    difficulty: media
  - id: sociales_estado_2
    type: opcion_multiple
    question: "¿Quién investiga a un alcalde corrupto?"
    options:
      - "La Contraloría"
      - "La Procuraduría"
      - "La Defensoría"
      - "El Congreso"
    correct_answer: 1
    explanation: "La Procuraduría vigila y sanciona a los funcionarios públicos, incluyendo alcaldes y gobernadores."
    difficulty: facil
  - id: sociales_estado_3
    type: opcion_multiple
    question: "El presidente quiere cambiar una ley, ¿puede hacerlo solo?"
    options:
      - "Sí, el presidente tiene poder absoluto"
      - "No, debe ir al Congreso (Rama Legislativa)"
      - "Sí, pero solo en casos de emergencia"
      - "Depende del tipo de ley"
    correct_answer: 1
    explanation: "No, el presidente no puede cambiar leyes solo. Debe ir al Congreso (Rama Legislativa) porque hacer y reformar leyes es función exclusiva de esa rama."
    difficulty: facil
---

# Estado y organización política (ICFES)

## ¿Qué evalúa el ICFES aquí?
Las ramas del poder público. Quién hace qué en Colombia.

## Conceptos clave (lo que sí o sí debo saber)
- **Rama Legislativa (Congreso):** Hace las leyes y reforma la constitución. (Senado y Cámara).
- **Rama Ejecutiva (Presidente, Alcaldes):** Administra y ejecuta las leyes. Maneja la plata.
- **Rama Judicial (Jueces, Cortes):** Juzga y castiga. Resuelve conflictos.
- **Organismos de Control:** Procuraduría (vigila funcionarios), Contraloría (vigila dinero), Defensoría (Derechos Humanos).

## Cómo suele preguntar el ICFES
- "El presidente quiere cambiar una ley, ¿puede hacerlo solo?" (No, debe ir al Congreso).
- "¿Quién investiga a un alcalde corrupto?" (Procuraduría).

## Ejemplo tipo ICFES (explicado)
**Enunciado:** Un ciudadano cree que una ley viola sus derechos fundamentales. ¿A qué rama debe acudir?
A) Ejecutiva.
B) Legislativa.
C) Judicial.
D) Electoral.

**Análisis:**
- C: **Correcta.** Los jueces (Corte Constitucional) deciden si una ley es válida o viola derechos (Tutela).

## Resumen para memorizar
- Legislativa = Leyes.
- Ejecutiva = Obras/Administración.
- Judicial = Justicia.
- Pesos y contrapesos: Ninguna rama manda sobre las otras.