---
id: sociales_democracia
title: Democracia y participación ciudadana
area: sociales
order: 4
type: lesson
published: true
description: Conoce los mecanismos de participación ciudadana: voto, plebiscito, referendo, cabildo abierto y revocatoria del mandato. Diferencia cada uno y su uso apropiado.
xp: 500
coins: 250
questions:
  - id: sociales_democracia_1
    type: opcion_multiple
    question: "El presidente quiere consultar al pueblo si está de acuerdo con un tratado de paz. ¿Qué mecanismo usa?"
    options:
      - "Referendo"
      - "Plebiscito"
      - "Consulta Popular"
      - "Cabildo abierto"
    correct_answer: 1
    explanation: "El plebiscito es para apoyar o rechazar una decisión del ejecutivo (presidente). El presidente pregunta SÍ o NO sobre una decisión."
    difficulty: media
  - id: sociales_democracia_2
    type: opcion_multiple
    question: "Los ciudadanos están descontentos con el alcalde porque no cumplió sus promesas. ¿Qué pueden hacer?"
    options:
      - "Nada"
      - "Revocatoria del mandato"
      - "Plebiscito"
      - "Referendo"
    correct_answer: 1
    explanation: "La revocatoria del mandato permite echar a un alcalde o gobernador (NO al presidente) por incumplir el plan de gobierno."
    difficulty: facil
  - id: sociales_democracia_3
    type: opcion_multiple
    question: "¿Cuál es la diferencia entre plebiscito y referendo?"
    options:
      - "No hay diferencia"
      - "Plebiscito: pregunta del presidente (SÍ/NO), Referendo: aprobar o derogar normas/leyes"
      - "Referendo: pregunta del presidente, Plebiscito: aprobar leyes"
      - "Ambos son iguales"
    correct_answer: 1
    explanation: "Plebiscito: el presidente pregunta SÍ o NO sobre una decisión. Referendo: aprobar o derogar normas/leyes. Cambiar Constitución = Referendo."
    difficulty: facil
---

# Democracia y participación ciudadana (ICFES)

## ¿Qué evalúa el ICFES aquí?
Mecanismos de participación. No solo es votar.

## Conceptos clave (lo que sí o sí debo saber)
- **Voto:** Elegir representantes.
- **Plebiscito:** El presidente pregunta SÍ o NO sobre una decisión (Ej: Paz).
- **Referendo:** Aprobar o derogar normas/leyes.
- **Cabildo Abierto:** Reunión pública con concejales/alcalde.
- **Revocatoria del mandato:** Echar a un alcalde o gobernador (NO al presidente) por incumplir el plan de gobierno.

## Cómo suele preguntar el ICFES
- "Los ciudadanos están descontentos con el alcalde porque no cumplió sus promesas. ¿Qué pueden hacer?" (Revocatoria).

## Ejemplo tipo ICFES (explicado)
**Enunciado:** El presidente quiere consultar al pueblo si está de acuerdo con un tratado de paz. ¿Qué mecanismo usa?
A) Referendo.
B) Plebiscito.
C) Consulta Popular.
D) Cabildo abierto.

**Análisis:**
- B: **Correcta.** El plebiscito es para apoyar o rechazar una decisión del ejecutivo.

## Resumen para memorizar
- Alcalde malo = Revocatoria.
- Cambiar Constitución = Referendo.
- Pregunta del Presidente = Plebiscito.